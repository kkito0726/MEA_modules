...
def draw(
    ...
    # 電極座標を取得
    ele_point = np.array(
        [[np.ravel(xx_vec)[i], np.ravel(np.flipud(yy_vec))[i]] for i in range(64)]
    )
    ...
    for f in range(len(time_del[0])):
        ...
        result = griddata(
            points=knew_xy_coord, values=knew_values, xi=(xx, yy), method="cubic"
        )
        result -= np.min(result)
        result_vec = griddata(
            points=knew_xy_coord,
            values=knew_values,
            xi=(xx_vec, yy_vec),
            method="cubic",
        )
...
# ベクトル解析で伝導速度を算出
def calc_velocity_from_grid(
    data: ndarray, peak_index: ndarray, ele_dis=450, mesh_num=8
) -> List[ndarray]:
    time_del, remove_ch = remove_undetected_ch(data, peak_index)
    ...
    # 電極座標を取得
    ele_point = np.array(
        [[np.ravel(xx_ele)[i], np.ravel(np.flipud(yy_ele))[i]] for i in range(64)]
    )
