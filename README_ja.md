...
```python
# 保存 (dtypeで容量と精度を選択)
mea.save_npz("data.npz")                   # float32 (実質無損失, 約1/2 + 圧縮)
mea.save_npz("data.npz", dtype="int16")    # int16 (16bit量子化, 約1/4 + 圧縮)
...
| dtype | 精度 | 目安サイズ | 用途 |
|---|---|---|---|
| `"float32"` (既定) | 実質無損失 (誤差 < ADC分解能) | 小 | 通常はこちら |
| `"int16"` | 16bit量子化 (誤差 < scale) | 最小 | 最大限軽くしたいとき |
