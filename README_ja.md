...
*   [MEA計測データの読み込み](#mea計測データの読み込み)
    ...
    *   [データの保存・再読み込み (.npz)](#データの保存再読み込み-npz)
*   [ピーク抽出](#ピーク抽出)
*   [グラフ描画](#グラフ描画)
    *   [64電極波形描画](#64電極波形描画)
    *   [1電極波形描画](#1電極波形描画)
    *   [ピークをプロット](#ピークをプロット)
    *   [拍動周期ごとの波形を重ね合わせて描画](#拍動周期ごとの波形を重ね合わせて描画)
...
2. [Python3のインストール](https://www.python.org/downloads/release/python-31211/) (3.12.11 推奨 ※devcontainerと同一バージョン)
3. PyMEAのインストール
...
### データの保存・再読み込み (.npz)
MEA計測データは容量が大きいため、解析の中間結果を圧縮した `.npz` で保存して再利用できる。
電位データは内部的に float32 で保持しているため、`.hed`/`.bio` から読み直すより高速・軽量に扱える。

```python
# 保存 (dtypeで容量と精度を選択)
mea.save_npz("data.npz")                     # int16 (既定。最小サイズ)
...
| dtype | 精度 | 目安サイズ | 用途 |
|---|---|---|---|
| `"int16"` (既定) | 16bit量子化 (誤差 < 半ADC-LSB＝測定分解能以下) | 最小 | 通常はこちら |
| `"float32"` | ビット完全一致 (無損失) | 小 | 厳密に元の値を保持したいとき |

> 注: int16 の誤差は測定器の分解能(1 LSB)の半分未満(信号振幅に対し約1/65000、典型値で0.0N μV程度)で、
> ピーク検出・ISI・FPD等の解析結果に影響しない。
> **振幅の実数値そのものが重要な研究テーマ(振幅の絶対値比較・微小変化の定量など)では `dtype="float32"` を使うこと。**

> **⚠️ 重要**: `.npz` は元データから派生した軽量コピー(int16は非可逆)。**計測生データ(`.hed`/`.bio`)は必ずバックアップとして保持し、削除しないこと。**

詳細(API・容量の目安・区間切り出しの活用・仕組み)は [.npz 保存・読み込みドキュメント](./docs/api/npz_io_ja.md) を参照。

> 💡 **複数ファイルを一括変換したい場合**: Python不要の単一バイナリCLI **[mea2npz](./tools/mea2npz/README.md)** を使うと、フォルダ内の `.hed`/`.bio` をまとめて `.npz` に変換できます (pyMEA生成物と数値一致)。はじめての方は [かんたんマニュアル（初心者向け）](./docs/mea2npz_manual.md) を参照してください。
>
> ```bash
...
各拍動の1stピークを基準に波形を切り出し、時刻を0始まりに揃えて重ね描きする。拍動ごとの波形のばらつきや、FPDの第2ピークの位置を一目で確認したいときに便利。
...
ch = 3                               # 重ね合わせる電極番号
before_sec = 0.05                    # 1stピークの何秒前から切り出すか
after_sec = 0.3                      # 1stピークの何秒後まで切り出すか

sr = mea.data.SAMPLING_RATE          # サンプリングレート (Hz)
peak_index_neg = detect_peak_neg(mea.data)

# 秒数をフレーム数に変換 (from_slice の引数は「フレーム番号」のため)
before_frame = int(before_sec * sr)
after_frame = int(after_sec * sr)

# 各1stピークの前後を切り出し、時刻を0始まりに揃える
mea_list = [
    mea.from_slice(
        index - before_frame,
        index + after_frame,
...
> `mea.from_beat_cycles(peak_index_neg, base_ch=ch)` でも拍動周期ごとのMEAリストを取得できる (前後対称のマージン)。1stピークの前後を非対称に切り出したい場合は上記のように `from_slice` を使う。
...
Claude Codeのプロンプトで `/pyMEA-codegen` に続けて解析内容を指示します。

```
/pyMEA-codegen データを読み込んでch6のISIを計算してグラフ表示してください
```
