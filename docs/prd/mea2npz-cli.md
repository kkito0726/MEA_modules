...
`<input>` は **`.hed` ファイル**（変換）/ **ディレクトリ**（一括変換）/ **`.npz` ファイル**（情報表示）。

- `.hed`／ディレクトリ → 変換（これまでの動作）。**単一 `.hed` 変換時は、変換前にその計測メタ情報（電極間距離・サンプリングレート・GAIN・計測時間(s)）を `.npz` 情報表示と同じ表形式で出力**してから変換する（4.6）
- **`.npz` → 情報表示モード**：電極間距離・サンプリングレート・GAIN・計測時間(s) を表形式で出力（4.6）

**引数なしで起動した場合は対話モード**に入り、入力パス・dtype・区間・電極間距離・時刻リセット・
（ディレクトリ時）再帰探索を順に尋ねる（各項目は Enter で既定値）。対話で集めた設定は
フラグ経路と同じ変換フローへ合流する（4.5）。対話で `.npz` を指定した場合は情報表示に切り替わる。
...
`mea2npz` を引数なしで実行したときは、標準入力から対話的に設定を集める。Python を持たない
利用者がフラグを覚えなくても、画面の案内に従うだけで変換できる。

- 質問順: 入力パス（必須）→ dtype →**「読み込み時間を指定する」(Y/n, 既定Y)** → 指定する場合は **開始秒・終了秒を必須**で入力 / 指定しない場合は全区間 → 電極間距離 → 時刻リセット（既定Y）→ 出力先（空で既定）→（入力がディレクトリなら）再帰探索。
- 各項目は **Enter で既定値**を採用（既定はフラグと同一）。ただし時間を指定する場合の開始秒・終了秒は**既定値なしの必須入力**（整数になるまで再入力、EOF で中断）。入力パスも必須で、空のまま EOF なら中断（終了コード非0）。
- 対話で組み立てた設定はフラグ経路と**同一の `options` に正規化**され、同じ単一変換/一括変換フロー（`dispatch`）へ合流する（ロジック重複なし）。
- 対話入出力は presentation（`interface/cli`）に閉じる。stdin/stdout を注入可能にし、設定組み立て（`collectOptions`）を実行（`dispatch`）から分離してテスト可能にする。

### 4.6 計測メタ情報の表示（`.npz` 情報表示モード / 単一 `.hed` 変換時）

`.npz` ファイルを入力に渡すと、変換せずに**計測メタ情報を表形式で表示**する。Python なしで
ファイルの素性（電極間距離・サンプリングレート・GAIN・計測時間）を確認できる。
**単一 `.hed` を変換するときも、変換に先立って同じ表でその計測情報を表示**する（利用者が
変換対象の素性をその場で確認できる）。`.hed` は電極間距離をファイルに持たないため、`-distance`
（既定 450）の値を表示する。表の描画ロジックは両モードで共通化する。
...
- 表示項目: 電極間距離（`.npz` の旧形式で欠落時は `N/A` / `.hed` は `-distance` の指定値）/ サンプリングレート / GAIN / 計測時間 (= end − start) / dtype（`.hed` 変換前表示では `.npz` 確定前のため省略可）。
- **電位データ本体（voltages）は読まず**、メタ情報だけをデコードする（軽量）。`.npz` は ZIP 内のスカラ `.npy`、`.hed` は `.hed` ヘッダ + `.bio` の総バイト数（フレーム数算出）から取得する。
- pyMEA 生成の `.npz` でも同様に読める（相互運用、検証済み）。
- アーキテクチャ: domain に `MeasurementInfo` 値オブジェクト。`.npz` は `MeasurementInfoReader` ポートを infra（`npz`）が `.npy` デコードで実装し、usecase `InspectUseCase` が取得する。`.hed` は電極間距離がファイル外（ユーザ指定）でポートの引数なし `Info()` と signature が異なるため、`hedbio` リーダの `Info(distance)` から取得する。**表の描画（CJK 幅考慮）は presentation（cli）で両モード共通**。表示幅計算は presentation に閉じる。

### 4.7 Go アーキテクチャ設計（DDD / クリーンアーキテクチャ）
...
- FR-13: **対話モード**。引数なしで起動した場合は標準入力から対話的に設定（入力パス・dtype・区間・電極間距離・時刻リセット・再帰探索）を集め、フラグ経路と同一フローへ合流する（4.5）。各項目は Enter で既定値、入力パスのみ必須。
- FR-14: **.npz 情報表示モード**。入力が `.npz` ファイルのときは変換せず、電極間距離・サンプリングレート・GAIN・計測時間(s)（+dtype）を**表形式で表示**する（4.6）。電位データ本体は読まずメタ情報のみをデコードし、pyMEA 生成の `.npz` も読める。
- FR-15: **単一 `.hed` 変換時の計測情報表示**。単一 `.hed` ファイルを変換するとき、変換に先立って `.npz` 情報表示と同じ表形式でその計測メタ情報（電極間距離（`-distance` 指定値）・サンプリングレート・GAIN・計測時間(s)）を表示する（4.6）。電位データ本体は読まず、`.hed` ヘッダと `.bio` の総バイト数からメタ情報を取得する（軽量）。情報取得に失敗しても変換は継続する。
...
- NFR-5: **DDD / クリーンアーキテクチャ**で構成し、依存方向を内向き（domain ← usecase ← infrastructure/interface）に保つ。フォーマット詳細は infrastructure に隔離する（4.7）。`go vet` および依存方向を守る（必要なら import 制約 lint）。
